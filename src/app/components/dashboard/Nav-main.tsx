...
export function NavMain() {
  ...
  // Auto-open parent menu if a sub-item is active
  useEffect(() => {
    items.forEach((item) => {
  ...
  // Check if Dashboard/Overview link should be active
  // It should ONLY be active when on exactly /dashboard, not on sub-pages
  const isDashboardActive = pathname === '/dashboard'

  // Check if a regular menu item (without sub-items) should be active
  const isMenuItemActive = (item: NavItem) => {
    // If the item has sub-items, check if any sub-item is active
    if (item.items && item.items.length > 0) {
      return false // Parent items shouldn't be "active", only their children
    }
    
    // For regular items, check exact match or if it's a parent path
    // But exclude /dashboard to prevent it from staying active
