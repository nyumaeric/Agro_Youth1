...
self.addEventListener('fetch', (event) => {
  ...
  // CRITICAL: Skip all Next.js internal and navigation requests for speed
  if (
    url.pathname.startsWith('/_next/') ||
    url.pathname.startsWith('/__nextjs') ||
    url.pathname === '/_error' ||
    url.searchParams.has('_rsc') || // Skip React Server Components
    request.headers.get('RSC') || // Skip RSC requests
    request.mode === 'navigate' || // Let Next.js handle all navigation
    url.pathname.includes('/api/auth/') ||
    url.pathname === '/api/revalidate'
  ) {
    // Pass through without caching for instant navigation
    return;
  }

  // Handle API requests - Network First (fast timeout)
  if (url.pathname.startsWith('/api/')) {
    event.respondWith(
      Promise.race([
  ...
  // For everything else, just use network (no caching for speed)
  // This includes CSS, JS, and other assets that Next.js manages
});
